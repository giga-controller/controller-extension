...
export async function getPlatformDetails({integration, whitelistedUrls}: getPlatformDetailsProps): Promise<PlatformDetails> {
  const currentUrl = await getCurrentTabUrl();
  const platform: Platform | undefined = Object.entries(whitelistedUrls).find(([_, urlList]) =>
    urlList.some(url => currentUrl.includes(url))
